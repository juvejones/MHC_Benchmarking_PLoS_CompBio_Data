...
def WriteMat(f, key, mat_srcc, i):
	...
	for line in f:
		...
		if cols[1] == key:
			found = True
			try:
				mat_srcc[i].append(float(cols[2]))
			except:
				mat_srcc[i].append(0)
	if not found:
		mat_srcc[i].append(0)
...
methods = ["smm","smmpmbec","ann","NetMHC4","PickPocket","consensus","NetMHCpan2.8","NetMHCpan3","NetMHCcons", "mhcflurry","mhcflurry_pan"]
mat_srcc = []
allele_dict1 = {}
allele_dict2 = {}
allele_dict3 = {}
...
for i,method in enumerate(methods):
	mat_srcc.append([])
	with open(os.path.join(__location__, "../", method, "Rdata/allele_stat_srcc.txt"),'r') as f:
		for key, value in allele_dict1.iteritems():
			WriteMat(f, key, mat_srcc, i)
			f.seek(0)
