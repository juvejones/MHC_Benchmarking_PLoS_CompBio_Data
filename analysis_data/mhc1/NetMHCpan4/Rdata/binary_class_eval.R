...
## Look for matching filenames, open files, and do necessary operations
	#pdf("NetMHCpan4_allele_roc.pdf", width=4,height=4)
	for (i in 1:length(dat[[1]])){
		print(paste(" Process Allele: ", dat[[1]][i], sep = ""))
		if (file.exists(paste("HLA-", dat[[1]][i], ".NetMHCpan4.txt", sep = ""))){
			datafile <- read.table(paste("HLA-", dat[[1]][i], ".NetMHCpan4.txt", sep = ""),header=FALSE, sep=",",
                                            col.names=c("idx","peptide","meas","pred"), comment.char="#", skip=1)
		}
		else{
			next
		}
...
all_allele_bi_eval_plot <- function(methods){
  svg("methods_roc_new_r12.svg", width=4.5,height=4.5)
  ...
  for (i in 1:length(methods)){
    ...
    if (i == 1){
      plot(perf,main="ROC Curves - Enlarged",xaxis.lwd=1.5,yaxis.lwd=1.5,xaxis.cex.axis=1,yaxis.cex.axis=1,col=cols[i],lwd=1.5,cex.lab=1,add=F, ylim=c(0.7,1.0))
    }
    else {
      plot(perf,main="ROC Curves - Enlarged",xaxis.lwd=1.5,yaxis.lwd=1.5,xaxis.cex.axis=1,yaxis.cex.axis=1,col=cols[i],lwd=1.5,cex.lab=1,add=T, ylim=c(0.7,1.0))
    }
  }
  ##generate the random prediction line
  x_ran <- seq(0,1,length=10)
