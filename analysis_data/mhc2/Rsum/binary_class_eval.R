...
cols2 <- colorRampPalette(brewer.pal(n=6,name="Set3"))(6)
  ...
  for (j in 1:length(methods)){
    ...
    for (i in 1:length(dat[[1]])){
      ...
      if (file.exists(infile)){
        datafile <- read.table(infile,header=TRUE)
        if (tag == 0){
          firstplot <- TRUE
        }
        tag <- tag + 1
...
all_allele_bi_eval_plot <- function(methods){
  ...
  for (i in 1:length(methods)){
    ...
    ## Three of the methods use RANK as affinity meas and should be taken care of
    if (method == "consensus" || method == "comblib" || method == "tepitope"){
      pred <- prediction((1-as.numeric(data$predict_rank)), as.numeric(data$meas_bi))
    }
    else{
    ...
    ## Three of the methods use RANK as affinity meas and should be taken care of
    if (method == "consensus" || method == "comblib" || method == "tepitope"){
      rocobj <- roc(as.numeric(data$meas_bi),(1-as.numeric(data$predict_rank)),ci=TRUE,of="auc")
...
if(length(args)<1){
	write("ERROR: Invalid arguments supplied.\n 
		Usage: '--args allele_file=\"allele.txt\" output_file=\"allele_stat.txt\"'", stderr())
...
methods_all = c("NetMHCIIpan", "nn_align", "smm_align", "consensus", "comblib", "tepitope")
